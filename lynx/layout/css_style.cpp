...
void CSSStyle::SetValue(const std::string& name, const std::string& value) {
...
double CSSStyle::ClampWidthInner(double width) const {
    if (width < min_width_) width = min_width_;
    if (width > max_width_) width = max_width_;
...
double CSSStyle::ClampHeightInner(double height) const {
    if (height < min_height_) height = min_height_;
