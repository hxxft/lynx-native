...
namespace lynx {
struct NamedColor { const char *const name; const CSSColor color; };
...
CSSColor CSSColor::Parse(const std::string& css_str) {
...
unsigned int CSSColor::Cast() const {
    unsigned int color = (0xffffffff & b_) | ((0xffffffff & g_) << 8) | ((0xffffffff & r_) << 16) | ((0xffffffff & ((unsigned char)(a_ * 255))) << 24);
    return color;
}
}
