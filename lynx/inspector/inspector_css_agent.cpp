...
#include "inspector/default_css_style.h"
...
namespace {
    ...
    Json::Value getComputedStyle(lynx::RenderObject* root, uint64_t id) {
        int count = 0;
        lynx::RenderObject* renderer = findRenderer(root, id);
        lynx::CSSStyle* style = renderer->GetStyle();
        Json::Value propertys(Json::arrayValue);
        propertys[count++] = makeStyleProperty("height", renderer->measured_size_.height_);
        propertys[count++] = makeStyleProperty("width", renderer->measured_size_.width_);
