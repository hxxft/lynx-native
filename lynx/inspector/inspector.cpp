...
namespace debug {
    Inspector::Inspector(base::Thread* js_thread)
    : js_thread_(js_thread),
      inspector_thread_(lynx_new base::Thread(base::MessageLoop::MESSAGE_LOOP_IO, "InspectorThread")){
        inspector_thread_->Start();
    }
    
    void Inspector::Attach(jscore::Runtime* runtime, const std::string& address, int port) {
        runtime_ = runtime;
        ...
        inspector_thread_->Looper()->PostTask(base::Bind(&Inspector::AttachToInspectorThread, ref, address, port));
    }
    
    void Inspector::Send(Json::Value content) {
        base::ScopedRefPtr<Inspector> ref(this);
        std::string domain;
        inspector_thread_->Looper()->PostTask(base::Bind(&Inspector::SendToInspectorThread, ref, domain, content));
    }
    ...
    void Inspector::SendToInspectorThread(const std::string& command_type, Json::Value content) {
    ...
    void Inspector::DispatchMessage(const base::ScopedRefPtr<base::IOBuffer>& data_buffer, uint64_t size) {
        base::ScopedRefPtr<Inspector> ref(this);
        std::string message(data_buffer->data(), size);
        js_thread_->Looper()->PostTask(base::Bind(&Inspector::DispatchMessageToJSThread, ref, message));
    }
