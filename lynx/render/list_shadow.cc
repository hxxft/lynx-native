...
namespace lynx {
ListShadow::ListShadow(const char* tag_name,
                       uint64_t id,
                       RenderTreeHost* host)
    : View(tag_name,
           LYNX_LISTSHADOW,
...
void ListShadow::Layout(int left, int top, int right, int bottom) {
    View::Layout(left, top, right, bottom);
}

void ListShadow::InsertChild(ContainerNode* child, int index) {
    CellView* cell = lynx_new CellView(impl()->thread_manager(), render_tree_host());
