...
RenderTreeHostImplAndroid::RenderTreeHostImplAndroid(
  ...
  java_render_tree_host_.Reset(
      nullptr, Java_LynxRenderTreeHostImpl_create(
...
void RenderTreeHostImplAndroid::BeginFrame() {
  Java_LynxRenderTreeHostImpl_beignFrame(base::android::AttachCurrentThread(),
...
void RenderTreeHostImplAndroid::PrepareCommit() {
  Java_LynxRenderTreeHostImpl_prepareCommit(
      base::android::AttachCurrentThread(), java_render_tree_host_.Get());
}

RenderTreeHostImpl* RenderTreeHostImpl::Create(
    jscore::ThreadManager* thread_manager,
    RenderTreeHost* host,
    RenderObjectImpl* root) {
  return lynx_new RenderTreeHostImplAndroid(thread_manager, host, root);
}
