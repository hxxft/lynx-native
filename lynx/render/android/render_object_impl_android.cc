...
void FreeJavaRef(JNIEnv* env, jobject jcaller, jlong nativePtr) {
  lynx::RenderObjectImplAndroid* impl =
      reinterpret_cast<lynx::RenderObjectImplAndroid*>(nativePtr);
  if (impl != NULL) {
...
void ConstructJavaRef(JNIEnv* env,
                      jobject jcaller,
                      jlong nativePtr,
                      jobject object) {
  lynx::RenderObjectImplAndroid* impl =
...
RenderObjectImplAndroid::RenderObjectImplAndroid(jscore::ThreadManager* manager,
                                                 RenderObjectType type)
    : RenderObjectImpl(manager, type) {
  JNIEnv* env = base::android::AttachCurrentThread();
  render_object_impl_java_impl_.Reset(
      nullptr,
      Java_RenderObjectImpl_create(env, type, reinterpret_cast<long>(this)));
}
...
void RenderObjectImplAndroid::UpdateStyle(const lynx::CSSStyle& style) {
  if (!render_object_impl_java_impl_.IsNull()) {
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_RenderObjectImpl_updateStyle(env, render_object_impl_java_impl_.Get(),
...
void RenderObjectImplAndroid::SetPosition(const base::Position& position) {
  if (!render_object_impl_java_impl_.IsNull()) {
    ...
    Java_RenderObjectImpl_setPosition(
        env, render_object_impl_java_impl_.Get(),
...
void RenderObjectImplAndroid::SetSize(const base::Size& size) {
  if (!render_object_impl_java_impl_.IsNull()) {
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_RenderObjectImpl_setSize(env, render_object_impl_java_impl_.Get(),
...
void RenderObjectImplAndroid::InsertChild(RenderObjectImpl* child, int index) {
  ...
  if (!render_object_impl_java_impl_.IsNull()) {
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_RenderObjectImpl_insertChild(env, render_object_impl_java_impl_.Get(),
...
void RenderObjectImplAndroid::RemoveChild(RenderObjectImpl* child) {
  ...
  if (!render_object_impl_java_impl_.IsNull()) {
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_RenderObjectImpl_removeChild(env, render_object_impl_java_impl_.Get(),
...
void RenderObjectImplAndroid::SetAttribute(const std::string& key,
...
void RenderObjectImplAndroid::RequestLayout() {
  if (!render_object_impl_java_impl_.IsNull()) {
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_RenderObjectImpl_requestLayout(env,
...
void RenderObjectImplAndroid::SetData(
    int key,
    base::ScopedPtr<jscore::LynxValue> value) {
  if (!render_object_impl_java_impl_.IsNull()) {
    JNIEnv* env = base::android::AttachCurrentThread();
    if (value.Get() != NULL &&
        value->type_ == jscore::LynxValue::Type::VALUE_PLATFORM) {
      Java_RenderObjectImpl_setData(env, render_object_impl_java_impl_.Get(),
                                    key,
...
void RenderObjectImplAndroid::Animate(
  ...
  if (!render_object_impl_java_impl_.IsNull()) {
    ...
    Java_RenderObjectImpl_animate(env, render_object_impl_java_impl_.Get(),
                                  jkeyframes.Release(), joptions.Release());
  }
}
...
void RenderObjectImplAndroid::CancelAnimation(const std::string& id) {
  if (!render_object_impl_java_impl_.IsNull()) {
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_RenderObjectImpl_cancelAnimation(env,
                                          render_object_impl_java_impl_.Get());
  }
}

void RenderObjectImplAndroid::DispatchEvent(JNIEnv* env,
                                            jstring event,
                                            jobjectArray args) {
  ...
  if (render_object_weak_ptr_.IsValid()) {
    render_object_weak_ptr_->DispatchEvent(event_str, array);
  }
}
...
RenderObjectImplAndroid::GetImagePixel(int x, int y, int w, int h) {
  ...
  if (native_bitmap_.Get() != NULL) {
    ...
    if (AndroidBitmap_lockPixels(env, native_bitmap_.Get(),
                                 (void**)&pixels_data)) {
      return ret;
    }
    int column;
...
RenderObjectImpl* RenderObjectImpl::Create(jscore::ThreadManager* manager,
                                           RenderObjectType type) {
  return lynx_new RenderObjectImplAndroid(manager, type);
}
