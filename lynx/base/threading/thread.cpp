...
Thread::Thread(MessageLoop::MESSAGE_LOOP_TYPE type, const std::string& name) : message_loop_(type), thread_name_(name) {
    if(type == MessageLoop::MESSAGE_LOOP_UI) {
        message_loop_.BindToCurrentThread();
    }
}
    
Thread::Thread(MessageLoop::MESSAGE_LOOP_TYPE type) : message_loop_(type), thread_name_() {
    if(type == MessageLoop::MESSAGE_LOOP_UI) {
        message_loop_.BindToCurrentThread();
    }
}
