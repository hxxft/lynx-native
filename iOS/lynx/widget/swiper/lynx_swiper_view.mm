...
@interface LxSwiperView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
- (void)reloadData {
    ...
    for (NSInteger i = 0; i < [_dataSource numberOfViewsInSwiperView:self]; ++i) {
        ...
        size.height = size.height < childSize.height ? childSize.height : size.height;
