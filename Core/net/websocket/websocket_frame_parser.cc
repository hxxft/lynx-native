...
namespace net {
    ...
    websocket_error_(kWebSocketNormalClosure) {
        std::fill(masking_key_.key_,
                  masking_key_.key_ + WebSocketFrameHeader::kMaskingKeyLength,
                  '\0');
    }
    ...
    bool WebSocketFrameParser::Decode(
        ...
        while (current_read_pos_ < buffer_.size()) {
            ...
            base::ScopedPtr<WebSocketFrameChunk> frame_chunk =
...
//        WebSocketFrameHeader::kMaskingKeyLength;
    ...
    void WebSocketFrameParser::DecodeFrameHeader() {
        ...
        bool final_ = (first_byte & kFinalBit) != 0;
        ...
        if (masked) {
            if (end - current < kMaskingKeyLength)
                return;
            std::copy(current, current + kMaskingKeyLength, masking_key_.key_);
            current += kMaskingKeyLength;
        } else {
            std::fill(masking_key_.key_, masking_key_.key_ + kMaskingKeyLength, '\0');
        }
        
        current_frame_header_.Reset(new WebSocketFrameHeader(opcode));
        current_frame_header_->final_ = final_;
        current_frame_header_->reserved1_ = reserved1;
        current_frame_header_->reserved2_ = reserved2;
        current_frame_header_->reserved3_ = reserved3;
        current_frame_header_->masked_ = masked;
        current_frame_header_->payload_length_ = payload_length;
        current_read_pos_ += current - start;
        //DCHECK_EQ(0u, frame_offset_);
    }
    
    base::ScopedPtr<WebSocketFrameChunk> WebSocketFrameParser::DecodeFramePayload(
                                                                             bool first_chunk) {
        ...
        int next_size = static_cast<int>(
                                         std::min(static_cast<uint64_t>(buffer_.size() - current_read_pos_),
                                                  current_frame_header_->payload_length_ - frame_offset_));
        
        base::ScopedPtr<WebSocketFrameChunk> frame_chunk(new WebSocketFrameChunk);
        if (first_chunk) {
            frame_chunk->header = current_frame_header_->Clone();
        }
        frame_chunk->final_chunk = false;
        if (next_size) {
            frame_chunk->data = new base::IOBufferWithSize(static_cast<int>(next_size));
            char* io_data = frame_chunk->data->data();
            memcpy(io_data, &buffer_.front() + current_read_pos_, next_size);
            if (current_frame_header_->masked_) {
        ...
        if (frame_offset_ == current_frame_header_->payload_length_) {
