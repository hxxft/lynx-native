...
class Inspector : public InspectorSession::Dispatcher,
                  public base::RefCountPtr<Inspector> {
 public:
  Inspector(base::Thread* js_thread, const std::string& name);

  void Attach(jscore::Runtime* runtime,
 ...
 private:
  ...
  void SendToInspectorThread(const std::string& command_type,
                             Json::Value content);
