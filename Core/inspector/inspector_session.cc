...
InspectorSession::InspectorSession(Dispatcher* dispatcher)
  ...
  //            basic_info_.SetAppID("com.lynx.debugsample");
  //            basic_info_.SetPageID("00000000-68e7-2645-a041-539300000000_com.lynx.debugsample_26867_1");
  //            basic_info_.SetPageTitle("android-lynx-page");
...
void InspectorSession::Start(const std::string& address, int port) {
  if (socket_.Get() == nullptr) {
    socket_.Reset(
        new net::WebSocket(new InspectorSocketListener(this), address, port));
  }
  socket_->Connect();
}

void InspectorSession::Send(const std::string& command_type,
...
void InspectorSession::Dispatch(
    const base::ScopedRefPtr<base::IOBuffer>& data_buffer,
    uint64_t size) {
  dispatcher_->DispatchMessage(data_buffer, size);
}

void InspectorSocketListener::OnOpen(
    const base::ScopedRefPtr<base::IOBuffer>& data_buffer,
    uint64_t size) {
  if (delegate_) {
    delegate_->Send("register", "aaa");
    // delegate_->Send("{\"fromType\":\"device\",\"deviceType\":\"android\",\"commandType\":\"devicesAndPages\",\"resultStatus\":\"success\",\"devicesList\":[{\"udid\":\"00000000-68e7-2645-a041-539300000000\",\"deviceName\":\"Nexus
    // 5\",\"apps\":[{\"appId\":\"com.lynx.debugsample\",\"pages\":[{\"pageId\":\"00000000-68e7-2645-a041-539300000000_com.lynx.debugsample_26867_1\",\"pageTitle\":\"android-lynx-page\",\"clientWS\":\"\",\"debugUrl\":\"http://localhost:9526/inspector/inspector.html?ws=localhost:9526/devtools/page/android/MDAwMDAwMDAtNjhlNy0yNjQ1LWEwNDEtNTM5MzAwMDAwMDAwL2NvbS5tb2d1amllLnhjZGVidWdzYW1wbGUvMDAwMDAwMDAtNjhlNy0yNjQ1LWEwNDEtNTM5MzAwMDAwMDAwX2NvbS5tb2d1amllLnhjZGVidWdzYW1wbGVfMjY4NjdfMS9hbmRyb2lkLXhjb3JlLXBhZ2U=&remoteFrontend=1\"}]}]}]}");
  }
}

void InspectorSocketListener::OnMessage(
    const base::ScopedRefPtr<base::IOBuffer>& data_buffer,
    uint64_t size) {
  if (delegate_) {
